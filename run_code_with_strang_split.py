...
x = 2*np.pi*Lx*np.arange(-int(Nx/2), int(Nx/2))/Nx
dx = x[1]-x[0]
...
#initial conditions
env0  = funcs.soliton(e*x, 0, vals_env)
...
env0  = square(e*x, 0, vals_env)
u0    = e*funcs.NLS_approx(x, 0, vals_NLS, square)
v0    = e*funcs.NLS_approx_dt(x, 0, vals, square)
"""

plotnum = 1       #plot number
plotgap = 400     #distance between plots
...
vals_rk4 = [dx, ac]
vals_strang = [e, c, v1, v2, v3, k1, k2]
...
while t < tf:
    ...
    u_try = e*funcs.NLS_approx(x, t, vals_NLS, funcs.soliton)


    u_approx  = env*funcs.Exp(x, t, k, w)*np.exp(1j*(t*e**2)*0.5)
    ...
    if (n%plotgap)==0:
        ...
        plt.plot(x, np.real(u_try),'r-', label = 'analytical')
